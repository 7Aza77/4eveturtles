...
func (r *EventPostgres) Create(ctx context.Context, event entity.Event) (int64, error) {
	var id int64
	query := `INSERT INTO events (title, description, date, location, max_participants, creator_id, organization_id, group_chat_link)
			  VALUES ($1, $2, $3, $4, $5, $6, $7, $8) RETURNING id`
	err := r.db.QueryRowContext(ctx, query,
		event.Title, event.Description, event.Date, event.Location,
		event.MaxParticipants, event.CreatorID, event.OrganizationID, event.GroupChatLink,
	).Scan(&id)
	return id, err
}
...
func (r *EventPostgres) List(ctx context.Context, filter EventFilter) ([]entity.Event, error) {
	...
	// Join event_tags if filtering by tags
	...
	if filter.OrganizationID != nil {
		conditions = append(conditions, fmt.Sprintf("e.organization_id = %d", *filter.OrganizationID))
	}
	if len(filter.TagIDs) > 0 {
		...
		conditions = append(conditions, fmt.Sprintf("et.tag_id IN (%s)", strings.Join(tagStrs, ",")))
	}
	if len(conditions) > 0 {
	...
	if filter.SortBy != "" {
		...
		query += fmt.Sprintf(" ORDER BY e.%s %s", filter.SortBy, order)
	} else {
...
func (r *EventPostgres) Update(ctx context.Context, event entity.Event) error {
	query := `UPDATE events SET title=$1, description=$2, date=$3, location=$4, max_participants=$5,
			  organization_id=$6, group_chat_link=$7, updated_at=CURRENT_TIMESTAMP
			  WHERE id=$8`
	_, err := r.db.ExecContext(ctx, query,
		event.Title, event.Description, event.Date, event.Location, event.MaxParticipants,
		event.OrganizationID, event.GroupChatLink, event.ID,
	)
	return err
}
